...
createTest('dispatch(jump(n, byIndex, n, state))', routes, [], async ({ dispatch, snap }) => {
  await dispatch({ type: 'SECOND' })
...
describe('dispatch(set())', () => {
  ...
  createTest('dispatch(set(action, n))', routes, [], async ({ dispatch, snap }) => {
    await dispatch({ type: 'SECOND' })
    await snap(set({ state: { foo: 'bar' } }, -1)) // n is the relative entry index number
  })

  createTest('dispatch(set(action, keyString))', routes, [], async ({ dispatch, snap }) => {
    await dispatch({ type: 'SECOND' })
    await snap(set({ hash: 'bar' }, '345678')) // all keys are '345678' but `entries.findIndex` finds the FIRST entry
  })

  createTest('dispatch(set(action, index, byIndex === true))', routes, {
    basenames: ['/bar']
  }, [], async ({ dispatch, snap }) => {
    await dispatch({ type: 'SECOND' })
...
describe('dispatch(setParams())', () => {
  createTest('dispatch(setParams(params))', routes, [
    setParams({ foo: 'bar' })
  ])
...
describe('dispatch(setQuery())', () => {
  createTest('dispatch(setQuery(query))', routes, [
    setQuery({ foo: 'bar' })
  ])
...
describe('dispatch(setHash())', () => {
  createTest('dispatch(setHash(setHash))', routes, [
    setHash('new-hash')
  ])

  createTest('dispatch(setHash(func))', routes, [
    setHash(hash => 'new-hash')
  ])
})

describe('dispatch(setBasename())', () => {
  createTest('dispatch(setBasename(basename))', routes, {
    basenames: ['/new-basename']
  }, [
    setBasename('/new-basename')
  ])

  createTest('dispatch(setBasename(func))', routes, {
    basenames: ['new-basename']
  }, [
